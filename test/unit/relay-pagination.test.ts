...
describe("Unit | Relay pagination", function () {
  ...
  describe("is Relay type", function () {
    test("if Relay connection", function () {
      expect(isRelayType(connectionType as unknown as GraphQLType)).toBe(true);
    });

    test("if Relay edge type", function () {
      expect(isRelayType(edgeType as unknown as GraphQLType)).toBe(true);
    });
    ...
    test("if Relay page info type", function () {
      ...
      expect(isRelayType(pageInfoType as unknown as GraphQLType)).toBe(true);
    });
    ...
    test("if non Relay type", function () {
      ...
      expect(isRelayType(nonRelayType as unknown as GraphQLType)).toBe(false);
      expect(isRelayType(otherNonRelayType as unknown as GraphQLType)).toBe(
        false
      );
    });
  });

  test("can determine if a type is a Relay connection", function () {
    expect(isRelayConnectionType(connectionType)).toBe(true);
    expect(isRelayConnectionType(edgeType)).toBe(false);
  });

  test("can determine if a type is a Relay edge", function () {
    expect(isRelayEdgeType(edgeType)).toBe(true);
    expect(isRelayConnectionType(edgeType)).toBe(false);
  });

  test("can separate Relay pagination arguments", function () {
    const args = {
      first: 10,
      last: 10,
      after: "12345",
